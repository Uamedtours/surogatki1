...
<div class="vertical-item model-item overflow-hidden <?php echo esc_attr( $absolute_css_class ); ?>">
	...
	<div class="item-content text-left">
		...
		<?php endif; ?>
		<div class="model-name links-darkgrey">
				...
				<ul class="no-bullets mt-0">
					<?php foreach ( $model_parameters as $parameter_title => $parameter_value ) : ?>
						<?php if ( !empty( $parameter_title ) && ! empty( $parameter_value ) && empty( $atts['hide_' . $parameter_title] ) ) : ?>
							<li class="parameter">
								<span class="parameter-title fs-12 text-uppercase"><?php echo esc_html( ucfirst( str_replace( array( '-', '_' ), ' ', $parameter_title ) ) ); ?></span>
								<span class="parameter-value fs-12 fw-500 text-uppercase color-darkgrey"><?php echo esc_html( $parameter_value ); ?></span>
							</li>
						<?php endif; ?>
					<?php endforeach; ?>
				</ul>
		...
		<div class="model-footer">
			<a class="btn-link2" href="<?php the_permalink(); ?>"><?php esc_html_e('View profile', 'modelicom') ?></a>
