...
<div class="qtip-rating-html">
	<span class="title-tip"><?php echo round($rating['average'], 2); ?> <?php echo sprintf(__('Based on %s Votes', 'modelicom'), $rating['count']); ?></span>
	<ul class="list-note">
		<?php foreach ($rating['stars'] as $star => $info) : ?>
			<li>
				<span class="note"><?php echo esc_html($star); ?> <i class="fa fa-star"></i></span>
				<div class="wrap-bar">
					<span class="rating-bar"></span>
					<span class="rating-bar-progress" style="width: <?php echo esc_attr($info['as_percentage']); ?>%;"></span>
				</div>
				<span class="total"><?php echo esc_html($info['count']); ?></span>
			</li>
		<?php endforeach; ?>
		<?php
		?>
