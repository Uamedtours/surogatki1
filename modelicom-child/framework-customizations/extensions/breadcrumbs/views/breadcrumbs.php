...
if ( ! empty( $items ) ) : 
	$items = apply_filters('fw_ext_breadcrumbs_items', $items);
?>
	<ol class="breadcrumb">
		<?php for ( $i = 0; $i < count( $items ); $i ++ ) : ?>
			<?php if ( $i == ( count( $items ) - 1 ) ) : ?>
				<li class="breadcrumb-item last-item"><span><?php echo wp_kses_post( $items[ $i ]['name'] ); ?></span></li>
			<?php elseif ( $i == 0 ) : ?>
				<li class="breadcrumb-item first-item">
				<?php if ( isset( $items[ $i ]['url'] ) ) : ?>
					<a href="<?php echo wp_kses_post( $items[ $i ]['url'] ); ?>"><span><?php echo wp_kses_post( $items[ $i ]['name'] ); ?></span></a></li>
				<?php else : echo wp_kses_post( $items[ $i ]['name'] ); endif; ?>
				<?php
			else : ?>
			<li class="breadcrumb-item <?php echo esc_attr( $i - 1 ) ?>-item">
				<?php if ( isset( $items[ $i ]['url'] ) ) : ?>
