...
<div class="<?php echo esc_attr( $class ); ?>" <?php echo wp_kses_post( $data_animation ); ?>>
	<?php
	if ( ! empty( $atts['column_padding'] ) || ! empty( $atts['background_color'] ) || ! empty( $atts['column_height'] ) ) : ?>
	<div class="<?php echo esc_attr( $atts['column_padding'] . ' ' . $atts['background_color'] . ' ' . $atts['column_height'] ); ?>">
	<?php endif; //column_padding

		//showing column content
		echo do_shortcode( $content );

	if ( ! empty( $atts['column_padding'] ) || ! empty( $atts['background_color'] ) || ! empty( $atts['column_height'] ) ) : //closing optional column_padding div ?>
	</div>
	<?php endif; ?>
</div><!-- .col-* -->
